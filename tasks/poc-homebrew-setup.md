...
- Create the public tap repository `xiaoland/homebrew-braid`.
- Create a GitHub Release `v0.1.0` on `xiaoland/braid` with the packaged macOS arm64 tarball.
- Write `Formula/braid.rb` with the release URL and checksum.
- Verify `brew install xiaoland/braid/braid` works and `braid --version` prints the expected version.

### Step 2 — `braid setup` skeleton
- Add a new `Setup` command to the CLI (`src/cli.rs`).
- Add an interactive module (`src/setup.rs`) that:
  ...
  - Writes the PEM to `~/.braid/braid-of-<owner>.pem`, the webhook secret to `~/.braid/webhook_secret` or an env file, and the TOML config to `~/.braid/braid.toml`.
...
- Generate default `issue-pi` / `pr-pi` profiles (or Codex if configured) in the generated TOML.
- Run `braid doctor` equivalent diagnostics inline and print a concise report.
- Print the final `braid serve --config ~/.braid/braid.toml` command.

### Step 5 — Documentation
- Update `README.md` with `brew install xiaoland/braid/braid` and `braid setup --repository owner/repo`.
