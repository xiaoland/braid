...
- **Next Step — Adjusted Handoff Plan**:
  1. **Rerun Slice 6 tunnel-owned candidate/death/restore journey** once the account-less Cloudflare Quick Tunnel route is reachable; preserve public health/App-webhook evidence.
  2. **Fix and land the uncertain-write acceptance harness** in Slice 7. The current `connect_delay_proxy.py` on branch `handoff/slice7-wip` only delays traffic; it must instead drop the client-side response after the request has reached GitHub, so the writer observes a transport timeout and the existing uncertain-comment recovery adopts the already-created comment. Re-evaluate whether `GitHubError::Response` should ever map to `uncertain`; revert was taken because `Response` is a deterministic shape error.
  3. **Run a Context-unavailable fixture** in Slice 7 (e.g., a Profile with a hard byte budget too low for the fixture, or a permission/pagination boundary) to prove the `context-too-large` / `context-unavailable` Operational Status Comment and zero provider turn behavior.
  4. **Run the configured stable-direct-actor and unconfigured-shared-identity variants** from acceptance journey 5 as part of Slice 7; verify that the existing `agent_actor_node_ids` / `agent_attributions` suppression works and that a write under an unconfigured identity is processed as an external Wake/invalidation.
  5. **Run the complete `docs/10-prd/acceptance.md` campaign three consecutive times** using a fresh real fixture. Fix failures in the owning slice/test, then restart the three-run sequence; do not weaken the oracle or substitute internal state/logs for public evidence.
  6. **Release closure**: after three clean runs, update Product Truth only for proven behavior, finalize `CHANGELOG.md`, tag/release the macOS arm64 artifact, open the Linux x86_64 follow-on task, delete this packet, and leave no Python/PDM residue.
