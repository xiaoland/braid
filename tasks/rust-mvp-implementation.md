...
- **Next Step — Route A: Realign and prepare PR #94 for acceptance**:
  1. **Maintain this task packet** (done in this edit): reframe the goal around PR #94, record the baseline, and park Slice 7 external-fault harnesses on `handoff/slice7-wip`.
  ...
  3. **Update PR #94 body**: rewrite the description to state the current schema (v11), list the completed slices/worktrees/PR lifecycle, and explicitly list the only remaining blocker: three clean full acceptance campaigns using `docs/10-prd/acceptance.md`.
