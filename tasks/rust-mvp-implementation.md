...
- **Current Truth**: Product, Context, lifecycle, Codex, GitHub, SQLite/OTel/deployment, and acceptance designs are closed in their durable owners. Read-only research verified current GitHub GraphQL Issue/PR selections and pagination on live public objects. A temporary Rust 1.93/Tokio client initialized Codex 0.147, created two distinct persistent threads, and injected complete Markdown Context into both. A Rust OpenTelemetry 0.32 probe exported 945/10,000 traces at a 10% parent-based ratio and retained all controlled full-payload attributes. Rust SDK sampling is head sampling; result-aware tail sampling is an optional external Collector deployment, not MVP runtime logic. The existing Python package implements the withdrawn product and has no compatibility or state-migration promise.
- **Next Step**: Execute Slice 0 only: replace the Python build/runtime surface with the Rust package, version/config/migration/OTel/CLI skeleton, and packaged clean-install smoke. Do not begin Slice 1 until Slice 0's gate is recorded.
