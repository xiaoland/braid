...
- **Slice 5 Evidence**: The schema-v11 `scripts/tests/50_issue_to_pr.sh` campaign passed without operator correction against closed Issue #62 and Draft PR #63. Concurrent `pr ensure` calls from one Implementation Request converged on one PR. The real Codex Agent pushed only `acceptance/slice5-20260815T074245Z.md` and published concise comments with exactly one generated attribution block. A real review comment proved review Wake; resolving its thread replaced idle PR Context whose human-readable thread identity was the file location, folded the resolved body, and preserved the worktree. An Associated-Issue description edit during a controlled active turn then proved ordinary debounce, active fencing, one continuation, current Issue content, and the same worktree. No `# Braid Event References` mirror appeared. Cleanup removed the temporary hook, closed Issue #62/PR #63, deleted the fixture branch, and removed the isolated runtime. This is bounded Slice 5 evidence, not full product acceptance.
- **Next Step**: Extend Slice 5 through configured/unconfigured direct `gh` origin behavior and PR close/reopen/merge finalization, then prove PR worktree/session restart recovery in Slice 6. Keep native assignment/unassignment, App-webhook bootstrap, active-turn Braid restart, and Context-unavailable acceptance explicitly unproven until a real fixture exists; do not manufacture passes. Product acceptance still requires three consecutive complete campaigns, not this bounded pass alone.
