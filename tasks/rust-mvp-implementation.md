...
- **Current Truth**: Product, Context, lifecycle, Codex, GitHub, SQLite/OTel/deployment, and acceptance designs are closed in their durable owners. Slices 0–3 have passed their real public-boundary campaigns. Slice 4 is implemented through operational convergence at forward-only schema v7: idle/active Hard Invalidation, fold/unfold/delete lifecycle, close/finalization/sleep/reopen, compatible physical-thread resume, assignment-generation-scoped Operational Status, and complete soft/hard Context pressure behavior are externally observable. Slice 5 has a bounded Issue-to-implementation vertical with real schema-v10 evidence: App-authored writes, comment-ID-keyed `pr ensure`, deterministic same-tree bootstrap, Draft/native association, deterministic PR Profile selection, distinct PR Codex session, generation-scoped worktree, exact pushed diff, Agent-authored PR prose, review/review-thread Context replacement, and debounced Cross-surface Hard Invalidation while preserving the PR worktree. The current schema-v11 candidate removes edge-scoped description digests in favor of one Issue-owned visible-description comparison state, removes GraphQL node IDs and Context Revision from Agent-facing references/diagnostics, labels review threads by canonical location, and projects `braid gh` results through semantic GitHub comment/PR fields without an internal receipt command. This candidate has passed local engineering and migration gates but not yet a new real GitHub campaign. The installed App is limited to `xiaoland/braid` with Issues, Pull requests, and Contents write plus mandatory Metadata read. Direct-origin variants and PR lifecycle remain unimplemented/unclaimed. An ordinary GitHub App is not a standard assignable user, so the PoC uses the first trusted `@braid` on a dormant Issue as ActivationIntent plus its initial Wake; native assignment/unassignment and App-webhook bootstrap remain honest capability gaps rather than fabricated passes. Context-unavailable handling is implemented but lacks a controlled real permission/pagination fixture. Braid still publishes no turn mirror.
...
- **Slice 5 Evidence**: Candidate `braid 0.1.0` SHA-256 `b5f72255e79935526fcbed6bf27c402a1ff74d5936842509579122d69aefa612` passed the expanded `scripts/tests/50_issue_to_pr.sh` without operator correction against closed Issue #57 and Draft PR #58. App comment `5300402604` became the Implementation Request; concurrent `pr ensure` calls converged on receipt `01a0038d-8726-7c63-8220-d5bb0949bdbf` and one PR. The real Codex Agent pushed only `acceptance/slice5-20260815T035248Z.md` and published concise comments with exactly one generated attribution block. Review comment `3788426042` and thread `PRRT_kwDOT1OPZs6Zd5-0` proved review Wake, resolved-thread body folding, and idle PR Context replacement into session `01a00393-b317-7a40-8dd7-4d22c2413521`; an Associated-Issue description edit during a controlled active turn then proved ordinary debounce, active fencing, one continuation, session `01a00394-3741-79f0-afee-33c68a1de9ba`, current Issue content, and unchanged worktree `runtime/worktrees/pr-58/pr-codex-g1`. No `# Braid Event References` mirror appeared. Cleanup removed the temporary hook, closed Issue #57/PR #58, deleted the fixture branch, and removed the isolated runtime. This is bounded Slice 5 evidence, not full product acceptance.
- **Next Step**: With explicit external-mutation authority, rerun the real Slice 5 campaign against the schema-v11 candidate and assert semantic Event References, identifier-free `braid gh` results, idempotent retries, review-thread location projection, and Cross-surface Hard Invalidation. Only after that passes should Slice 5 extend through configured/unconfigured direct `gh` origin behavior and PR close/reopen/merge finalization, followed by PR worktree/session restart recovery in Slice 6. Keep native assignment/unassignment, App-webhook bootstrap, active-turn Braid restart, and Context-unavailable acceptance explicitly unproven until a real fixture exists; do not manufacture passes. Product acceptance still requires three consecutive complete campaigns, not this bounded pass alone.
