...
- **Next Step**: Begin Slice 5 with the narrowest end-to-end vertical: App-authored `braid gh` write receipts and comment-ID-keyed `pr ensure`, then native association, deterministic PR Profile selection, isolated worktree provisioning, and one PR Agent. Keep native assignment/unassignment, App-webhook bootstrap, active-turn Braid restart, and Context-unavailable acceptance explicitly unproven until a real fixture exists; do not block the PR slice by manufacturing them.
