...
- **Next Step — Route A: Realign and prepare PR #2 for acceptance**:
  1. **Maintain this task packet** (done in this edit): reframe the goal around PR #2, record the baseline, and park Slice 7 external-fault harnesses on `handoff/slice7-wip`.
  2. **Rename the baseline branch**: `feat/docs-namespace-migration` → `feat/rust-working-memory` (or a name you choose), and update the remote PR head to point to the new branch.
  3. **Update PR #2 body**: rewrite the description to state the current schema (v11), list the completed slices/worktrees/PR lifecycle, and explicitly list the only remaining blocker: three clean full acceptance campaigns using `docs/10-prd/acceptance.md`.
  4. **Run acceptance when environment is available**: execute every journey in `docs/10-prd/acceptance.md` three consecutive times with a fresh real fixture. Fix failures in the owning slice/test and restart the sequence; do not weaken the oracle.
  5. **Release closure** (after acceptance): update Product Truth only for proven behavior, finalize `CHANGELOG.md`, tag/release the macOS arm64 artifact, open the Linux x86_64 follow-on task, delete this packet, and leave no Python/PDM residue.
