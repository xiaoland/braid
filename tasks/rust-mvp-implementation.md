...
- **Slice 5 Evidence**: Candidate `braid 0.1.0` passed the expanded `scripts/tests/50_issue_to_pr.sh` against Issue #43 and Draft PR #44. App comment `5294430310` became the Implementation Request; concurrent `pr ensure` calls converged on receipt `01a000aa-1c62-7ec2-b42b-41223b50bffa` and one PR. Public status showed PR Profile `pr-codex`, generation 1, one distinct provider session, and worktree `runtime/worktrees/pr-44/pr-codex-g1`. The real Codex Agent pushed exactly `acceptance/slice5-20260814T142520Z.md` and published one attributed PR comment; the PR contained no `# Braid Event References` mirror. Cleanup closed Issues #35/#37/#39/#41/#43 and PRs #36/#38/#40/#42/#44, removed all five fixture branches, and left no Braid service process. This is bounded evidence, not the full Slice 5 gate.
- **Next Step**: Extend the same real campaign through review/review-thread Event References, Associated-Issue Context changes and active-turn Cross-surface Hard Invalidation, configured/unconfigured direct `gh` origin behavior, and PR close/reopen/merge finalization. Then exercise PR worktree/session restart recovery in Slice 6. Keep native assignment/unassignment, App-webhook bootstrap, active-turn Braid restart, and Context-unavailable acceptance explicitly unproven until a real fixture exists; do not manufacture passes.
