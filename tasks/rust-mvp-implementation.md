...
- **Next Step**: Extend the real Slice 5 campaign through configured/unconfigured direct `gh` origin behavior and PR close/reopen/merge finalization, then exercise PR worktree/session restart recovery in Slice 6. Keep native assignment/unassignment, App-webhook bootstrap, active-turn Braid restart, and Context-unavailable acceptance explicitly unproven until a real fixture exists; do not manufacture passes. Product acceptance still requires three consecutive complete campaigns, not this bounded pass alone.
