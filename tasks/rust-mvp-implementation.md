...
- **Current Truth**: Product, Context, lifecycle, Codex, GitHub, SQLite/OTel/deployment, and acceptance designs are closed in their durable owners. Slice 0 is complete. Slice 1 has an implementation candidate: GitHub App JWT/installation authentication, typed bounded GraphQL pagination, stable Issue/PR/Project/relationship/comment/review snapshots, drift retries, deterministic Markdown Context, AST-based HTML-comment removal, Context Revision/pressure, Operational Status exclusion, and deletion tombstones backed by forward-only schema v2. Every embedded query passed GitHub's live schema against public `openai/codex` objects, strict Rust gates pass, and the clean packaged gate observes schema 0→2, schema 1→2 with a pre-v2 backup, and newer-schema refusal. The real App-backed controlled-object gate remains explicitly unavailable because no Braid App private key or disposable Issue/PR fixture currently exists; user-token reads are research evidence only and are not product acceptance. No webhook, scheduler, provider turn, reaction, or GitHub mutation runtime has begun.
- **Next Step**: Provision or receive separately authorized Braid App credentials and a controlled real Issue/PR fixture, then run `scripts/tests/10_context_projection.sh` through the packaged binary. Fix any observed shape/lifecycle defects and record the real evidence before declaring Slice 1 complete or beginning webhook scheduling/provider turns.
...
Gate: formatting/check/Clippy pass; a clean directory with no Python/PDM/Cargo on PATH runs version/config/migration/profile/status; a local OTLP receiver observes one full-payload trace at ratio 1.0 and no orphan spans at ratio 0; upgrading a schema-0 fixture to schema 1 and rejecting schema 2 are externally observable.
