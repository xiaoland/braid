...
- **Next Step**: After the account-less Cloudflare route cools down, rerun only the Slice 6 tunnel-owned candidate/death/restore journey and preserve its public health/App-webhook evidence. Keep the exact remote-success/response-loss uncertain-write journey, configured stable direct actor, native assignment/unassignment, and Context-unavailable fixture explicit for Slice 7 rather than manufacturing internal passes. Product acceptance still requires three consecutive complete campaigns.
