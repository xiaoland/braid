...
- **Next Step**: Complete the remaining Slice 4 lifecycle matrix on the accepted reset state machine: provider restart/status convergence and context-pressure Operational Status. Native unassignment remains unavailable for the ordinary GitHub App and must not be fabricated; exercise it only when a native assignable Agent App capability exists. Extend the same public helper only when each runtime seam exists, and keep every not-yet-run journey explicitly unaccepted. Preserve the unavailable native Agent App assignment and App webhook bootstrap branches as honest capability results.
