...
| 术语 | 含义 |
| --- | --- |
| Braid App | 安装到 GitHub 的 App、Issue assignment target、PR mention target 及稳定 Bot 写入身份；不是 Coding Agent。GitHub 是否支持将 Agent App 原生 assign 给 PR 仍需真实探针证明。 |
...
| Assignment Generation | Work Item 连续由 Braid 激活的一个生命周期；Issue 可由原生 Agent App assignment 开始，PR 由经验证的 PR Activation signal 开始。重新激活会 fence 旧 generation 的能力和输出。 |
...
| Agent Attribution | Braid App 代写评论时附加的稳定、面向 Human 的 Profile/Agent 标识；不依赖隐藏 marker 或内部 ID。 |
